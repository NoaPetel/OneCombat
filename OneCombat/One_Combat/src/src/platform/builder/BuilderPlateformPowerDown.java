...
import src.platform.actions.PlatformPowerDown;
import system.GameSystem;

public class BuilderPlateformPowerDown implements PlatformBuilder{
	
	Color color;
	
	public BuilderPlateformPowerDown(Color color) {
		this.color = color;
	}
	
	public BasicPlatform build() throws IOException{
		BasicPlatform result = new BasicPlatform(this.color);
		result.addComponents(new PlatformPowerDown(result, result.transform));
