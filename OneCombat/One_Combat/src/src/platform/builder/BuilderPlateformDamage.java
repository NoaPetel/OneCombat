...
import src.platform.actions.PlatformDamage;
import system.GameSystem;

public class BuilderPlateformDamage implements PlatformBuilder{
	Color color;
	
	public BuilderPlateformDamage(Color color) {
		this.color = color;
	}
	
	public BasicPlatform build() throws IOException{
		BasicPlatform result = new BasicPlatform(this.color);
		result.addComponents(new PlatformDamage(result, result.transform));
